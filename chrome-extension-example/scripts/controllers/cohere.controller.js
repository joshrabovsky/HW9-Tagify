...
const tweetsController = require("tweets.controller.js");
...
const labelAccount = async (req, res, user) => {
    try {
        let [userId, userName, userDescription] = await tweetsController.asyncGetTwitterUserId(user);
        ...
        return await asyncGetCohereGeneration(coherePrompt)
    } catch (error) {
        res.send(error);
    }
}
